...
int hash_phone_number( void *ptr, int max )
  // apply hash to last name of the record
{
  struct record *rec;
...
int comp_phone_number( const void *ptr1, const void *ptr2 )
  // compare two records based on last name
{
  const struct record *rec1, *rec2;
  int result;
  ...
  result = strcmp( rec1->phone_number ,rec2->phone_number );
#ifdef DEBUG
  printf( "strcmp( \"%s\", \"%s\" )=%d\n", 
          rec1->last_name, rec2->last_name, result );
#endif
...
int main( int argc, char **argv )
{
  ...
  struct HashTable *by_phone_number;
  int i;

  memsys = init(327688,512);
  ...
  by_phone_number = createTable(memsys, CAPACITY, sizeof(struct record), hash_phone_number, comp_phone_number );
  ...
  for (i=0;i<TOTAL_RECORDS;i++)
  {
   ...
   // printf("HERE");
    ...
    addElement(memsys, by_phone_number, record[i]);
  }
  printf( "Hashes built\n" );
  printops( memsys );
  printf( "\n" );
  ...
  printf("ACCURACY: %d\n", hashAccuracy(memsys, by_last_name));
  printf("ACCURACY: %d\n", hashAccuracy(memsys, by_last_name));
  printf("ACCURACY: %d\n", hashAccuracy(memsys, by_phone_number));
  printops( memsys );
  printf( "\n" );

  freeTable( memsys, by_last_name );
  freeTable( memsys, by_first_name );
  freeTable( memsys, by_phone_number);
